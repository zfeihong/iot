...
namespace gm.di
{
    public class DefaultCfgRegistrar : CfgRegistrarBase
    {
        public override void AddType(IServiceCollection services, Type type)
        {
            if (IsCfgRegistrationDisabled(type)) return;
        ...
        protected virtual Type GetRedirectedTypeOrNull(
            ...
            List<Type> allExposingServiceTypes)
        {
            if (allExposingServiceTypes.Count < 2) return null;

            if (exposingServiceType == implementationType) return null;

            if (allExposingServiceTypes.Contains(implementationType)) return implementationType;
